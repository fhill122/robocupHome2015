...
def single_arm_pick(limb,alpha):
    ...
    traj.add_point(ik_position_list(limb,(p1x+p4x)/2+1.2*offset_x,(p1y+p4y)/2+1.2*offset_y,Z_PICK+0.06,r_x,r_y,r_z,r_w), 5.0)
    #lower z position
    traj.add_point(ik_position_list(limb,(p1x+p4x)/2+1.2*offset_x,(p1y+p4y)/2+1.2*offset_y,Z_PICK,r_x,r_y,r_z,r_w), 6.0)
    #closer
    traj.add_point(ik_position_list(limb,(p1x+p4x)/2+0.7*offset_x,(p1y+p4y)/2+0.7*offset_y,Z_PICK,r_x,r_y,r_z,r_w), 7.0)
    traj.start()
    traj.wait(10.0)
    traj.clear(limb)
    ...
    traj.add_point(ik_position_list(limb,X_START,Y_START,Z_START,R_X,R_Y,R_Z,R_W), 5.5)
    traj.start()
    traj.wait()
