...
3) should pop up with message successfully opened port /dev/rfcomm1
...
void
initPort() {
  fd = open( "/dev/rfcomm1", O_RDWR| O_NOCTTY | O_NDELAY);
  /* Error Handling */
  if ( fd < 0 ) {
...
main (int argc, char **argv) {
  ...
  ros::init(argc,argv, "gripper_test1");
	...
	ros::Subscriber sub = n.subscribe("gripper_test1/request", 1, gripperCB);
	pub = n.advertise<utilities::gripperTestStatus>("gripper_test1/status", 1);
