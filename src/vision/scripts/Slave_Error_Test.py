...
def tray_orientation(tray_angle):
    ...
    y_rot = 90*math.pi/180.0
    pick_angle=0*math.pi/180.0
    ...
    if ( tray_angle > 0) : #positive tray angle means that from baxter view, the right arm will be further away from the base than the left arm
        #LEFT
        Ry_l = np.matrix([ [cos(y_rot), 0, sin(y_rot)] , [0, 1, 0] , [-sin(y_rot), 0, cos(y_rot) ] ])
        ...
        Rx2_l = np.matrix([[cos(pick_angle), -sin(pick_angle), 0],[sin(pick_angle), cos(pick_angle), 0],[0, 0, 1]])
		#RIGHT
        Ry_r = np.matrix([[cos(y_rot), 0, sin(y_rot)],[0, 1, 0],[-sin(y_rot), 0, cos(y_rot)]])
        ...
        Rx2_r = np.matrix([[cos(pick_angle), -sin(pick_angle), 0],[sin(pick_angle), cos(pick_angle), 0],[0, 0, 1]])
    else:
    ...
    R_r = Ry_r*Rz_r*Rx_r*Rx2_r #Rotation Matrix for Right end effector
    R_l = Ry_l*Rz_l*Rx_l*Rx2_l #Rotation Matrix for Left End Effector
...
def left_to_right(left_pose):
    
    q=left_pose['orientation']
    q2=Quat(q[3],q[0],q[1],q[2])
    v = Vector3(0, 0, 1)
    v2=q2*v
    v_norm=v2.normalized()
    ...
    #qr = Quat_Conj(q2.w,q2.x,q2.y,q2.z)
    #right_pose=[x2,y2,z2,qr[0],qr[1],qr[2],qr[3]]
    
    q_right=find_opposite(q2)
...
def Quat_Conj(w,x,y,z):
    #Give in (w,x,y,z) returns conjugate of q in (x,y,z,w)
    return [-x, -y, -z, w]
    
def find_opposite(q_left):
    
    
    q_rot=Quat.new_rotate_axis(math.pi,Vector3(0,0,1))
    
    q_new=q_rot*q_left
    
    return q_new
    
def tilt_down(q,down_angle):
    
    #reference end effector orientation
    
    v = Vector3(0, 0, 1)
    
    #current end effector vector
    v2=q*v
    
    #rotate by pi/2 about z
    
    q_rot=Quat.new_rotate_axis(math.pi/2,Vector3(0,0,1))
    
    v3=q_rot*v2
    
    #generate appropriate quaternion for current end effector angle
    
    q_down=Quat(down_angle,v3.x,v3.y,v3.z)
...
def main():
    ...
    for i in range(len(positions)-1):
        ...
        split_sequence=splitter(positions[i],positions[i+1])
        
        for a in range(len(split_sequence)-1):
                
            path.append(split_sequence[a])
    ...
    while (a):       
        ...
        i=0
        pick_sequence=list(tray_to_end_effector(path))    
        ...
        while hello==True:
            ...
            while c<len(path):
                ...
                pick_left=pick_sequence[1][c]
                print pick_left
                pick_right=pick_sequence[0][c]
                ...
                while not ((i>1.0)or(rospy.is_shutdown())):
                    ...
                    left_pose=arm_l.endpoint_pose()
                    right_pose=left_to_right(left_pose)
                    position=ik_test(pick_left,right_pose)
                    arm_r.set_joint_positions(position[1])
